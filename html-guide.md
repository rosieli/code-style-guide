...
#### [强制] 使用 `1/4` 个空格做为一个缩进层级，不允许使用 `tab` 字符。

解释：
- 对于非 HTML 标签之间的缩进，比如 script 或 style 标签内容缩进，与 script 或 style 标签缩进一级，style、html 和 script 模块之间用3行作为分隔。
- 样式以一个最外层容器样式，作为命名空间。
- 组件文件名用中横线书写，组件 name 以首字母大写的驼峰格式书写和文件名一致。
- 应用标签格式和 html 格式一致，以中横线书写 `<my-demo></my-demo>` 
...
<style>
  /* 样式内容，以一个最外层容器样式，作为命名空间 */
  .component-my-demo-container {
      padding: 0;
...
<template>
  <ul class="component-my-demo-container">
...
#### [建议] `id` 建议单词全字母大写，单词间以 `_` 分隔。同项目必须保持风格一致。eg: <div id="GOOGLE_CAPTCH"></div>
...
#### [建议] 模板代码的缩进优先保证 HTML 代码的缩进规则，并且在{{  }}里面左右分别留一个空格，能用 v-text 的尽量用 v-text.
...
#### [建议] 模板代码应以保证 HTML 单个标签语法的正确性为基本原则，并且在内容很长时，一个属性一行显示，HTML 相关内容在前面，vue数据相关在后面。
...
<div
  ...
  :class="{'disable-button' : isDisable}"
  @click="onVerifyButton"
  v-text="buttonShowText">
    <default-input
      class="FORM-input-group"
      v-model="phoneCaptchaCode"
      :label="$root.langData.captcha"
      :type="'tel'"
      :placeholder="$root.langData.captcha"
      :validateType="'captcha'"></default-input>
</div>

<!-- bad -->
<div class="captcha-button" :class="{'disable-button' : isDisable}" @click="onVerifyButton" v-text="buttonShowText">
    <default-input class="FORM-input-group" v-model="phoneCaptchaCode" :label="$root.langData.captcha" :type="'tel'" :placeholder="$root.langData.captcha" :validateType="'captcha'"></default-input>
</div>
```
