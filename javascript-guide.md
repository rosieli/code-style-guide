...
[1] https://github.com/ecomfe/spec/blob/master/javascript-style-guide.md

[2] https://github.com/ecomfe/spec/blob/master/es-next-style-guide.md
...
[4 环境](#user-content-4-%E6%B5%8F%E8%A7%88%E5%99%A8%E7%8E%AF%E5%A2%83)
...
## 1 前言
Javascript日常开发，市面上有各种较为成熟的规范，约束程度有松有紧，具体的规则条目数量众多，其中每一项的是否合理也是见仁见智。这份文档在其他团队（主要是规范[1]）规范的基础上进行的小改动，其中很多规范已经过时（比如AMD模块相关的东西），之后可以删掉，另外有一些规则感觉有待商榷，标记为[值得讨论]，可以商量一下再最终确定。

规范这种东西的存在一定程度上是开发习惯众口难调之下诞生的一个统一化方案。

最理想的方式是大家商量出一套都可以接受的公共规则及集合，写成eslint规范，在开发项目时能执行和落实。
...
##### [建议] 使用 `4` 个空格做为一个缩进层级。 当然有人就喜欢用2个空格也是没办法的~
...
##### [值得讨论] 在对象创建时，属性中的 `:` 之后必须有空格，`:` 之前不允许有空格。
...
##### [值得讨论] 在函数调用、函数声明、括号表达式、属性访问、`if / for / while / switch / catch` 等语句中，`()` 和 `[]` 内紧贴括号部分不允许有空格。
...
##### [值得讨论] 变量必须 `即用即声明`，不得在函数或其它形式的代码块起始位置统一声明所有变量。
